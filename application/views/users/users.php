                                    ...
                                    <table  class="table table-hover emg1" id="myTable">
                                      ...
                                      <?php if($users) foreach ($users as $key => $users): ?>
                                        <tr>
                                         ...
                                         <td><?php echo ($users->can_add == 1) ? "Yes" : "No" ;?></td>
                                         <td><?php echo ($users->can_edit == 1) ? "Yes" : "No" ;?></td>
                                         <td><?php echo ($users->can_delete == 1) ? "Yes" : "No" ;?></td>
                                         <td><?php echo ($users->user_status == 1) ? "Active" : "Not Active" ;?></td>
                                         <td>
                                           <?php if($this->session->userdata('role_id') == '1'){?>
                                            <a href="javascript:void(0);" data-id="<?php echo $users->id;?>"  data-first_name="<?php echo $users->first_name;?>"  data-last_name="<?php echo $users->last_name;?>"  data-email="<?php echo $users->email;?>"  data-user_status="<?php echo $users->user_status;?>"  data-location_id="<?php echo $users->location_id;?>"  data-role_id="<?php echo $users->role_id;?>"  data-image="<?php echo $users->image;?>"  data-can_add="<?php echo $users->can_add;?>"  data-can_edit="<?php echo $users->can_edit;?>"  data-can_delete="<?php echo $users->can_delete;?>" class="btn btn-info btn-sm item_edit" >Edit</a>
                                             <?php };?>
                                             <?php if($this->session->userdata('role_id') == '1'){?>
    ...
    $(document).ready(function(){
      ...
      $('#addUserform').validate({
                ...
                submitHandler: function(form) {
                        ...
                        $.ajax({
                            url : "<?php echo base_url(); ?>user/UserUser/add_user",  
                            type : 'POST',
                            data : formData,
                            async : false,
                            cache : false,
         ...
         $('#editUserform').validate({
                ...
                submitHandler: function(form) {
                        ...
                        $.ajax({
                            url : "<?php echo base_url(); ?>user/UserUser/update_user",  
                            type : 'POST',
                            data : formData,
                            async : false,
                            cache : false,
        ...
        //delete record to database
         $('#btn_userdelete').on('click',function(){
              ...
              $.ajax({
                  type : "POST",
                  url  : "<?php echo base_url(); ?>user/UserUser/delete_user",
                  dataType : "JSON",
                  data : {id:id},
                  success: function(data){
                      window.location.href =  window.location.href;
