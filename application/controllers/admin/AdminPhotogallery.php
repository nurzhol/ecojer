...
class AdminPhotogallery extends CI_Controller {
	
	public function __construct()
    {
        parent::__construct();
		$this->load->model('admin/PhotogalleryModel','PhotogalleryModel');
	...
	public function add_photogallery()
	{
        ...
        else
        {
			...
			if(isset($_FILES['image']['name']) && !empty($_FILES['image']['name'])){
	             $uploadPath = 'assets/backend/uploads/photogallery/';
	            ...
	            if(!$this->upload->do_upload('image')){
		        	...
		        	$data=$this->PhotogalleryModel->save_photogallery();
		        	if($data == 'title_exist') {
	...
	public function all_photogallery()
	{
		$isLoggedIn = $this->session->userdata('isLoggedIn');
        ...
        {
            $data['photogallery'] = $this->PhotogalleryModel->viewRecords();
          
			$this->load->view('admin/includes/header');
			$this->load->view('admin/includes/nav');
			$this->load->view('admin/photogallery/photogallery',$data);
        ...
        else
        {
            ...
            if(isset($_FILES['image']['name']) && !empty($_FILES['image']['name'])){
                ...
                if(!$this->upload->do_upload('image')){
                    ...
                    $fileData = $this->upload->data();
                    $_POST['image_name'] =$fileData['file_name'];
                    $data=$this->PhotogalleryModel->update_photogallery();
                    if($data == 'title_exist') {
                        echo json_encode(array("status"=>"title_exist","error"=>"title is already exist"));
                    }else if($data == 'false'){
                        echo json_encode(array("status"=>"false","error"=>"Getting Error While saving ,try again later"));
    ...
    public function delete_photogallery()
    {
        $data = $this->PhotogalleryModel->delete_photogallery();
